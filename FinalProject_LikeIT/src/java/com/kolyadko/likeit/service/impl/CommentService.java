...
public class CommentService extends AbstractService<Integer, Comment> {
    private static final String SELECT_GENERAL = "SELECT C.comment_id, author_id, question_id, text, creation_date, " +
            "answer, mark_num, C.rating, C.archive, login, avatar_id, first_name, last_name, " +
            "gender, password, birth_date, sign_up_date, email, role, state, U.rating, U.answer_num, " +
...
//        ConnectionProxy connection = getConnectionWrapper();
...
//    }
    ...
    public ArrayList<CommentData> findByQuestionId(Integer questionId, String login) throws ServiceException {
        if (login != null) {
            return findDataBy(SELECT_COMM_USER_MARK + WHERE_QUEST_ID + EXISTING + ORDER_BY_CREATE_DATE,
                    login, questionId);
        }
    ...
    private ArrayList<CommentData> findDataBy(String query, Object... params) throws ServiceException {
        try (ConnectionProxy connection = getConnectionWrapper()) {
    ...
    @Override
    protected ArrayList<CommentData> extractData(ConnectionProxy connection, ResultSet resultSet) throws ServiceException {
        CommentDao commentDao = new CommentDao(connection);
